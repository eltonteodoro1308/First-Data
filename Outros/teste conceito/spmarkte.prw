...
//Alimenta o array
	...
	oDespesBrw:SetDelete( .T., {|a,b,c,d,e,f| funcEdit(a,b,c,d,e,f) } )
	...
	oDespesBrw:addColumn({"Codigo"              , {||PadR( aDespes[oDespesBrw:nAt,02], 50)}, "C", "@!"    , 1, 20    ,  , .T. , , .F.,, "aDespes[oDespesBrw:nAt,02]",, .F., .T.,                                    , "ETDESPES1"    })
	oDespesBrw:addColumn({"Nome"                , {||PadR( aDespes[oDespesBrw:nAt,03], 50)}, "C", "@!"    , 1, 20    ,  , .T. , , .F.,, "aDespes[oDespesBrw:nAt,03]",, .F., .T.,                                    , "ETDESPES2"    })
	oDespesBrw:addColumn({"End"                 , {||PadR( aDespes[oDespesBrw:nAt,04], 50)}, "C", "@!"    , 1, 20    ,  , .T. , , .F.,, "aDespes[oDespesBrw:nAt,04]",, .F., .T.,                                    , "ETDESPES3"    })
	oDespesBrw:addColumn({"Cidade"              , {||PadR( aDespes[oDespesBrw:nAt,05], 50)}, "C", "@!"    , 1, 20    ,  , .T. , , .F.,, "aDespes[oDespesBrw:nAt,05]",, .F., .T.,                                    , "ETDESPES4"    })

	oDespesBrw:setEditCell( .T. , { |a,b,c,d,e,f| funcEdit(a,b,c,d,e,f) } ) //activa edit and code block for validation

    /*
    oDespesBrw:acolumns[2]:ledit     := .T.
    oDespesBrw:acolumns[2]:cReadVar:= 'aDespes[oBrowse:nAt,2]'*/
 
	setKey( VK_F3, {|| Alert('oDespesBrw:nAt'), Alert('oDespesBrw:ColPos()') } )
...
Return .T.
 ...
 static function funcEdit(a,b,c,d,e,f)

 //B:Colpos() -> COLUNA POSICIONADA
 //B:nAt -> LINHA POSICIONADA
